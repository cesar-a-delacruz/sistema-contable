...
public class ShadowPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (shadowTop || shadowBottom || shadowLeft || shadowRight && shadowSize > 0 && shadowOpacity > 0) {
            ...
            for (int i = 0; i < shadowSize; i++) {
                ...
                drawCustomRoundedRect(g2, x, y, w, h, topLeftArcShadow, topRightArcShadow, bottomLeftArcShadow,
                        bottomRightArcShadow);
            }
            addBorderForShadow();
        }
        ...
        drawCustomRoundedRect(g2, shadowLeft ? shadowSize : 0, shadowTop ? shadowSize : 0,
                width - (shadowLeft ? shadowSize : 0) - (shadowRight ? shadowSize : 0),
                height - (shadowTop ? shadowSize : 0) - (shadowBottom ? shadowSize : 0), topLeftArcVisible,
                topRightArcVisible, bottomLeftArcVisible, bottomRightArcVisible);
        ...
        if (borderColor != null && borderThickness > 0 && borderOpacity > 0) {
            ...
            drawCustomRoundedRect(g2, (shadowLeft ? shadowSize : 0) + borderThickness / 2.0, // Ajustar la posición para
                                                                                             // que el borde no se corte
                    ...
                    topLeftArcVisible, topRightArcVisible, bottomLeftArcVisible, bottomRightArcVisible);
        }
    ...
    private void addBorderForShadow() {
        this.setBorder(BorderFactory.createEmptyBorder(shadowTop ? shadowSize + borderThickness : 0,
                shadowLeft ? shadowSize + borderThickness : 0, shadowBottom ? shadowSize : 0,
                shadowRight ? shadowSize : 0));
    }
     ...
     * @param bottomRightArc
     *            Radio de la esquina inferior derecha.
     */
    private void drawCustomRoundedRect(Graphics2D g2, double x, double y, double width, double height, int topLeftArc,
            int topRightArc, int bottomLeftArc, int bottomRightArc) {
